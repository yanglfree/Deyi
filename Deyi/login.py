...
def get_image(driver,name):
    img=driver.find_element_by_class_name(name)
    time.sleep(2)
...
def crack(driver): #破解滑动认证
    ...
    # 2、获取没有缺口的图片
    image1 = get_image(driver,'gt_cut_bg')
    ...
    # 4、获取有缺口的图片
    image2 = get_image(driver,'gt_cut_fullbg')
    ...
    # 7、按照行动轨迹先正向滑动，后反滑动
    button = driver.find_element_by_class_name('gt_guide_tip')
...
def login_cnblogs(username,password):
    ...
    try:
        ...
        input_username = driver.find_element_by_id('clf_username')
        input_pwd = driver.find_element_by_id('clf_password')
        signin = driver.find_element_by_id('clf_submit')
...
if __name__ == '__main__':
    login_cnblogs(username='鲁班大师智商二百五',password='Yl040911')
